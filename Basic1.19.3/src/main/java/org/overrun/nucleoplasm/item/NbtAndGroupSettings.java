...
import org.overrun.nucleoplasm.records.StackRecord;
...
@Deprecated(since = "delete this using yml register this", forRemoval = true)
public enum NbtAndGroupSettings {
    ...
    public static final Map<String,StackRecord> stackRecords = new HashMap<>();
    ...
    public static void init() {
        DecayTimeSettings.init();
        for (NbtAndGroupSettings value : values()) {
            get(value);
        }
    }

    public static ItemStack getStack(NbtAndGroupSettings[] values, int neutron, String abbreviation) {
        for (NbtAndGroupSettings value : values) {
            if (abbreviation.equals(value.name())) {
                ItemStack stack = new ItemStack(RegItem.elementera.getItem().get());
                ...
                for (DecayTimeSettings decayTime : DecayTimeSettings.values()) {
                    String key = value.name() + neutron;
                    if (decayTime.name().equals(key)) {
                        tag.putBoolean("decay", decayTime.hasDecay());
                        tag.putDouble("relative_atomic_mass", decayTime.getRelative_atomic_mass());
                        if (decayTime.hasDecay()) {
    ...
    public static void get(NbtAndGroupSettings value) {
...
//            itemStack.setTag(tag);
    ...
    private static void panac(List<StackRecord.Record> list, String[] PANAC) {
        int count = Integer.parseInt(PANAC[1].trim());
        for (var v : values()) {
            //查找衰变删除记录成nbt
            int neutron1 = 0;
            boolean b = false;
            for (int i = 0; i < PANAC[0].length(); i++) {
                try {
                    Integer.parseInt(String.valueOf(PANAC[0].charAt(i)));
                    if (i == 0) {
                        i = Integer.parseInt(String.valueOf(PANAC[0].charAt(i)));
                    } else {
                        i = Integer.parseInt("%s%s".formatted(String.valueOf(i), String.valueOf(PANAC[0].charAt(i))));
                    }

                } catch (NumberFormatException e) {

                    if (v.name().length() > i) {
                        b = v.name().charAt(i) == PANAC[0].charAt(i);
                    } else {
                        b = false;
                    }
                }
            ...
            if (b) {
                for (var dtv : DecayTimeSettings.values()) {
                    if (dtv.name().equals(v.name() + neutron1)) {
                        list.add(
                            new StackRecord.Record(
                                v.proton, neutron1,
                                v.translate, v.name(),
                                dtv.hasDecay(), dtv.getRelative_atomic_mass(),
                                dtv.getHalf_life(), dtv.getMc_half_life()
                            )
                        );
                    }
                }
